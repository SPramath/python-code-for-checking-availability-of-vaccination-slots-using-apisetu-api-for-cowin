...
def is_eighteen_plus(session):
    return session["age_limit"] <= 44

def get_for_seven_days(start_date):
    url = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict"
    params = {"district_id": (district id), "date": start_date.strftime("%d-%m-%Y")}
...
username = "(gmail id 1)"
password = "(password of gmail id 1)"
user = "(email id 2)"
...
if not content:
    ...
    with smtplib.SMTP(host='smtp.gmail.com', port='587') as server:
        ...
        server.send_message(email_msg, user)